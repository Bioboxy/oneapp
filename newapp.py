...
sm = ScreenManager()
sm.add_widget(WelcomeScreen(name = 'welcomecreen'))
...
class NewApp(MDApp):
	...
	def check_username(self):
		...
		if username_check_false or self.username_text.split() == []:
			cancel_btn_username_dialogue = MDFlatButton(text = 'Retry', on_release = self.close_user_dialogue)
			self.dialog = MDDialog(title = 'Invalid', text = "Please enter a valid username", size_hint = (0.7, 0.2), buttons = [cancel_btn_username_dialogue])
			self.dialog.open()
	...
	def close_user_dialogue(self, obj):
		self.dialog.dismiss()
